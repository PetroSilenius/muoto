...
export default async function handler(
  ...
  if (!req.query.imageId) {
    const stableImage = await triggerStableDiffusion(session.user.id);
    return res.status(200).json(stableImage);
  }

  const image = await client.images.findFirstOrThrow({
    where: { user_id: session.user.id, id: req.query.imageId as string },
  });
